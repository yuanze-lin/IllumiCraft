...
from dataset import SimpleBatchSampler, VideoDatasetWithResizing, VideoDatasetWithResizeAndRectangleCrop, VideoDatasetWithResizingTracking  # isort:skip
...
from models import AutoencoderKLWan, WanT5EncoderModel, CLIPModel
...
def filter_kwargs(cls, kwargs):
    """Return only keyword arguments accepted by the target class constructor."""
    import inspect
...
# Generate validation samples by loading optional control inputs and running the pipeline.
def log_validation(
    ...
    """Prepare optional control inputs and run the pipeline to generate validation videos."""
    logger.info(
    ...
    # When tensors are not preloaded, read video files and apply resizing/normalization on the fly.
    if args.load_tensors is False:
        ...
        # Optional HDR and tracking maps are resized to the nearest bucket before normalization.
        if hdr_map_path:
        ...
        # The foreground video is always required for validation, so load it unconditionally here.
        ...
        # The background reference is optional and is reduced to its first frame during validation.
        ...
        # Move validation inputs to the model device and prepare them for inference.
        with torch.no_grad():
            if hdr_map_path:
            ...
            if background_map_path is not None:
                ...
                background_frames = background_frames[:, :, :1, :, :]
    ...
    # Keep validation generation light so it does not dominate training time.
    if args.num_validation_videos > 2:
        args.num_validation_videos = 2

    # Collect generated videos so they can be saved and returned together.
    videos = []
    ...
    # Write validation videos to disk for every configured tracker backend.
    ...
    # Clear unused GPU memory after validation to reduce fragmentation.
    torch.cuda.empty_cache()
    return videos


# Stack per-sample fields into a single batch for training.
class CollateFunctionImageTracking:
    def __init__(self, weight_dtype: torch.dtype, load_tensors: bool) -> None:
        """Store the batch dtype and whether prompts are already tensors."""
        self.weight_dtype = weight_dtype
        self.load_tensors = load_tensors

    def __call__(self, data: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        """Merge a list of dataset samples into one dictionary of batched tensors."""
...
# Set up the model, optimizer, dataloader, and training loop.
def main(args):
    if args.hub_token is not None:
        raise ValueError("This training script does not support hub push. Please unset --hub_token.")

    # MPS does not support bf16 mixed precision, so fail early with a clear message.
    ...
    # Load the experiment configuration before building any model or optimizer objects.
    if args.config_path:
    ...
    # Load the text tokenizer, scheduler, encoders, VAE, and transformer backbone.
    ...
    # Freeze the pretrained components and train only the transformer module.
    ...
    # Enable gradient checkpointing when requested to save memory.
    if args.gradient_checkpointing:
        transformer.enable_gradient_checkpointing()

    accelerator.register_save_state_pre_hook(
        partial(save_model_hook, accelerator, transformer, vae, text_encoder, tokenizer, clip_image_encoder)
    )
    accelerator.register_load_state_pre_hook(
        partial(load_model_hook, accelerator, transformer, args, config, load_dtype)
    )
    ...
    # Collect the trainable transformer parameters and report their names for debugging.
    ...
    # Build dataset arguments once and switch dataset classes depending on the available supervision.
    dataset_init_kwargs = {
    ...
    # Use a custom batch sampler so every batch follows the dataset bucket constraints.
    ...
    # Initialize trackers and log memory only from the main process.
    ...
    # Restore optimizer/model state when resuming, or start from step zero otherwise.
    ...
    # Main optimization loop over epochs and batches.
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # Let Accelerate handle gradient accumulation across microbatches.
            with accelerator.accumulate(models_to_accumulate):
                ...
                # Encode inputs and build control latents without tracking gradients.
                with torch.no_grad():
                ...
                # Convert prompts to embeddings unless they are already provided as tensors.
                ...
                # Sample noise and timesteps, then build the noisy training target.
                ...
                # Compute a masked MSE loss so extreme residuals do not dominate training.
                ...
                # Run the transformer on noisy latents plus all conditioning signals.
                ...
                loss = loss.mean()
                # Backpropagate the loss for the current accumulation step.
                accelerator.backward(loss)

                if accelerator.sync_gradients:
                    # Clip gradients only on the final microbatch of an accumulation window.
                    ...
                    # Step the optimizer and scheduler only when gradients are synchronized.
            ...
            # Update progress, checkpoint, and global step only after a real optimizer step.
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process or accelerator.distributed_type == DistributedType.DEEPSPEED:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        # Checkpoint save: handled by save_model_hook (transformer only).
        ...
        # Run validation from the main process after checkpoint intervals.
        if accelerator.is_main_process:
            if args.validation_prompt is not None and global_step % args.checkpointing_steps == 0:
                ...
                # Split the configured validation triplets into per-example lists.
                ...
                # Validate each prompt/video/background trio in order.
